...
/**
 * Planner that checks for modifications and plans to skip entities that
...
public class ModificationAwarePlanner implements SynchronizationPlanner {
    ...
    private static final List<String> CATALOG_KEYS_TO_IGNORE =
            List.of(
                    ...
                    // For certain storageConfigInfo fields, depending on the credentials Polaris was set up with
                    // to access the storage, some fields will always be different across the source and the target.
                    // For example, for S3 my source and target Polaris instances may be set up with different AWS users,
    ...
    private Map<String, Object> removeKeys(Map<String, Object> map, List<String> keysToRemove) {
        Map<String, Object> cleaned = objectMapper.convertValue(map, new TypeReference<Map<String, Object>>() {});
        ...
        for (String key : keysToRemove) {
            ...
            if (separateFirst.length > 1) {
                ...
                try {
                    Map<String, Object> subMap = objectMapper.convertValue(valueForPrimary, new TypeReference<>() {});
                    Map<String, Object> cleanedSubMap = removeKeys(subMap, List.of(separateFirst[1])); // remove nested keys from submap
    ...
    public SynchronizationPlan<PrincipalRole> planPrincipalRoleSync(List<PrincipalRole> principalRolesOnSource, List<PrincipalRole> principalRolesOnTarget) {
        ...
        SynchronizationPlan<PrincipalRole> delegatedPlan = delegate.planPrincipalRoleSync(
                sourceRolesByName.values().stream().toList(), targetRolesByName.values().stream().toList());
    ...
    private boolean areSame(Catalog source, Catalog target) {
        return areSame(source, target, CATALOG_KEYS_TO_IGNORE)
                // because of the way the jackson serialization works, any class that extends HashMap is serialized
                // with just the fields in the map. Unfortunately, CatalogProperties extends HashMap so we must
                // manually compare the fields in the catalog properties and cannot automatically deserialize them
                // as a map
    ...
    public SynchronizationPlan<Catalog> planCatalogSync(List<Catalog> catalogsOnSource, List<Catalog> catalogsOnTarget) {
        ...
        SynchronizationPlan<Catalog> delegatedPlan = delegate.planCatalogSync(
                sourceCatalogsByName.values().stream().toList(), targetCatalogsByName.values().stream().toList());
    ...
    public SynchronizationPlan<CatalogRole> planCatalogRoleSync(String catalogName, List<CatalogRole> catalogRolesOnSource, List<CatalogRole> catalogRolesOnTarget) {
        ...
        SynchronizationPlan<CatalogRole> delegatedPlan = delegate.planCatalogRoleSync(
                catalogName, sourceCatalogRolesByName.values().stream().toList(), targetCatalogRolesByName.values().stream().toList());
    ...
    public SynchronizationPlan<GrantResource> planGrantSync(String catalogName, String catalogRoleName, List<GrantResource> grantsOnSource, List<GrantResource> grantsOnTarget) {
        ...
        SynchronizationPlan<GrantResource> delegatedPlan = delegate.planGrantSync(
                catalogName, catalogRoleName, sourceGrants.stream().toList(), targetGrants.stream().toList());
