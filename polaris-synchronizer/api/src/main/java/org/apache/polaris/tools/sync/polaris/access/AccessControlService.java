...
/**
 * Service class to facilitate the access control needs of the synchronization. This involves setting up
 * principals, principal roles, catalog roles, and grants to allow the tool to be able to introspect
 * into catalog internals like catalog-roles, tables, grants.
 */
public class AccessControlService {
     ...
     * @param replace if true, if an existing omnipotent principal role exists, it will be dropped and recreated
    ...
    public PrincipalWithCredentials createOmnipotentPrincipal(boolean replace) {
        ...
        Principal omnipotentPrincipalPrototype = new Principal()
                .name(OMNIPOTENT_PRINCIPAL_NAME_PREFIX + System.currentTimeMillis())
                .putPropertiesItem(OMNIPOTENCE_PROPERTY, ""); // this property identifies the omnipotent principal

        for (Principal principal : principals) {
            if (principal.getProperties() != null && principal.getProperties().containsKey(OMNIPOTENCE_PROPERTY)) {
                if (replace) {
                    ...
                    throw new IllegalStateException(
                            "Not permitted to replace existing omnipotent principal, but omnipotent " +
                                    "principal with property " + OMNIPOTENCE_PROPERTY + " already exists");
                }
            }
        }
    ...
    public PrincipalRole getOmnipotentPrincipalRoleForPrincipal(String principalName) {
        List<PrincipalRole> principalRolesAssigned = polaris.listPrincipalRolesAssignedForPrincipal(principalName);

        return principalRolesAssigned
                .stream()
                .filter(
                        principalRole -> principalRole.getProperties() != null
                                && principalRole.getProperties().containsKey(OMNIPOTENCE_PROPERTY)
                )
                .findFirst()
                .orElseThrow(() -> new NoSuchElementException(
                        "No omnipotent principal role with property " + OMNIPOTENCE_PROPERTY + " exists for principal " + principalName));
    }

    /**
     * Creates a principal role for the omnipotent principal and assigns it to the provided omnipotent principal.
     * @param omnipotentPrincipal the principal to create and assign the role for
     * @param replace if true, drops existing omnipotent principal roles if they exist before creating the new one
     * @return the principal role for the omnipotent principal
     */
    public PrincipalRole createAndAssignPrincipalRole(PrincipalWithCredentials omnipotentPrincipal, boolean replace) {
        List<PrincipalRole> principalRoles = polaris.listPrincipalRoles();

        PrincipalRole omnipotentPrincipalRole = new PrincipalRole()
        ...
        for (PrincipalRole principalRole : principalRoles) {
            if (principalRole.getProperties() != null && principalRole.getProperties().containsKey(OMNIPOTENCE_PROPERTY)) {
                ...
                if (replace) {
                    ...
                    throw new IllegalStateException(
                            "Not permitted to replace existing omnipotent principal role, but omnipotent " +
                                    "principal role with property " + OMNIPOTENCE_PROPERTY + " already exists");
                }
            }
        }
    ...
    /**
     * Creates an omnipotent catalog role for a catalog and assigns it to the provided omnipotent principal role
     * @param catalogName the catalog to create the catalog role for
     * @param omnipotentPrincipalRole the omnipotent principal role to assign the created catalog role to
     * @param replace if true, drops and recreates the existing omnipotent catalog role
     * @return the created omnipotent catalog role
     */
    public CatalogRole createAndAssignCatalogRole(String catalogName, PrincipalRole omnipotentPrincipalRole, boolean replace) {
        List<CatalogRole> catalogRoles = polaris.listCatalogRoles(catalogName);

        for (CatalogRole catalogRole : catalogRoles) {
            if (catalogRole.getProperties() != null && catalogRole.getProperties().containsKey(OMNIPOTENCE_PROPERTY)) {
                if (replace) {
                    polaris.removeCatalogRole(catalogName, catalogRole.getName());
                } else {
                    throw new IllegalStateException(
                            "Not permitted to replace existing omnipotent catalog role for catalog " + catalogName +
                                    ", but omnipotent principal with property " + OMNIPOTENCE_PROPERTY + " already exists");
                }
            }
        }

        CatalogRole omnipotentCatalogRole = new CatalogRole()
     ...
     * @param withWriteAccess if the catalog role should be given write access to the catalog internals
     * @return the grants that were added to the catalog role
     */
    public List<CatalogGrant> addGrantsToCatalogRole(String catalogName, String catalogRoleName, boolean withWriteAccess) {
        if (withWriteAccess) {
            // write access only requires CATALOG_MANAGE_METADATA
            CatalogGrant catalogManageMetadata = new CatalogGrant()
            ...
            CatalogGrant catalogReadProperties = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(CATALOG_READ_PROPERTIES);

            CatalogGrant namespaceReadProperties = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(NAMESPACE_READ_PROPERTIES);

            CatalogGrant namespaceList = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(NAMESPACE_LIST);

            CatalogGrant tableReadProperties = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(TABLE_READ_PROPERTIES);

            CatalogGrant tableList = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(TABLE_LIST);

            CatalogGrant viewReadProperties = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(VIEW_READ_PROPERTIES);

            CatalogGrant viewList = new CatalogGrant()
                    .type(GrantResource.TypeEnum.CATALOG)
                    .privilege(VIEW_LIST);
            ...
            return List.of(catalogReadProperties, namespaceReadProperties, tableReadProperties, viewReadProperties);
        }
    }
    ...
    public boolean omnipotentCatalogRoleExists(String catalogName) {
        ...
        return catalogRoles.stream().anyMatch(
                catalogRole -> catalogRole.getProperties() != null
                        && catalogRole.getProperties().containsKey(OMNIPOTENCE_PROPERTY)
        );
    }

    /**
     * Creates catalog role for catalog, assigns it to provided principal role, and assigns grants with appropriate
     * privilege level
    ...
    public void setupOmnipotentRoleForCatalog(
        ...
        CatalogRole omniPotentCatalogRole = createAndAssignCatalogRole(catalogName, omnipotentPrincipalRole, replace);
